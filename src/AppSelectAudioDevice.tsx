...
function AppSelectAudioDevice() {
  ...
  useEffect(() => {
    getCurrentWebviewWindow().once<AudioDevice[]>('audio-devices', async ({ payload }) => {
              ...
              <>
                ...
                {audioDevices.map(({ id, name }) => (
                  <button
                    ...
                    onClick={async () => await getCurrentWebviewWindow().emit('selected', { Selected: id })}
                  >
                    
                    <MicIcon className="w-5 h-5 ml-4 mr-2" />
                    <span className="truncate text-left leading-tight text-sm">{name}</span>
                  </button>
                ))}
              </>
            )}
